...
const z = __mwReactJsxRuntime.jsx;
const B = __mwSdkReact.useSettings;
const G = __mwSdkReact.useViewport;
const H = __mwSdkReact.useSettingsActions;
const h = __mwReact.useRef;
const V = __mwReact.useMemo;
const O = __mwReact.useCallback;
const P = __mwReact.useEffect;
const W = {
  color: "#FFFFFF",
  speed: 0.28,
  origin: 0,
...
function Z() {
  ...
  const g = (o) => o * o * o * (o * (o * 6 - 15) + 10), m = (o, s, p, R) => {
    const d = ((s % 16 + 16) % 16 * 16 + (o % 16 + 16) % 16) * 2;
    return f[d] * p + f[d + 1] * R;
  };
  for (let o = 0; o < 256; o++)
    for (let s = 0; s < 256; s++) {
      const p = s / 256 * 16, R = o / 256 * 16, d = Math.floor(p), v = Math.floor(R), T = p - d, E = R - v, A = g(T), S = g(E), e = m(d, v, T, E), b = m(d + 1, v, T - 1, E), x = m(d, v + 1, T, E - 1), y = m(d + 1, v + 1, T - 1, E - 1), t = e + A * (b - e), U = x + A * (y - x), r = t + S * (U - t);
...
function re() {
  const i = B(), n = { ...W, ...i }, u = G(), { setValue: a, onButtonClick: f } = H(), g = h(null), m = h(null), o = h(null), s = h(null), p = h({}), R = h(0), d = h(performance.now()), v = h(n), T = h(u);
  ...
  return P(() => {
    ...
    s.current = b, e.bindTexture(e.TEXTURE_2D, b), e.texImage2D(e.TEXTURE_2D, 0, e.R8, 256, 256, 0, e.RED, e.UNSIGNED_BYTE, E), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_S, e.REPEAT), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_WRAP_T, e.REPEAT), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MIN_FILTER, e.LINEAR), e.texParameteri(e.TEXTURE_2D, e.TEXTURE_MAG_FILTER, e.LINEAR);
    const x = k(e, $, e.VERTEX_SHADER), y = k(e, j, e.FRAGMENT_SHADER);
    if (!x || !y) return;
    ...
    const U = () => {
      const r = m.current, w = o.current;
      if (!r || !w) return;
      const l = v.current, _ = g.current;
      if (!_) return;
      const C = Math.min(1, Math.max(0.25, l.quality ?? 1)), L = T.current, I = Math.max(L.dpr || 1, 1) * C, M = Math.max(1, Math.round((_.clientWidth || L.width || 1) * I)), D = Math.max(1, Math.round((_.clientHeight || L.height || 1) * I));
      ...
      r.uniform3f(c.u_color, N, X, q), r.uniform1f(c.u_speed, l.speed), r.uniform1f(c.u_origin, l.origin), r.uniform1f(c.u_direction, l.direction), r.uniform1f(c.u_scale, l.scale), r.uniform1f(c.u_brightness, l.brightness), r.uniform1f(c.u_opacity, l.opacity), r.uniform1f(c.u_detail, l.detail), r.uniform1f(c.u_turbulence, l.turbulence), r.uniform1f(c.u_density, l.density), r.uniform1f(c.u_height, l.height), r.uniform1f(c.u_quality, C), r.drawArrays(r.TRIANGLES, 0, 6), R.current = requestAnimationFrame(U);
    };
    return U(), () => {
      cancelAnimationFrame(R.current), o.current && (e.deleteProgram(o.current), o.current = null), s.current && (e.deleteTexture(s.current), s.current = null), m.current = null;
    };
  }, [E]), /* @__PURE__ */ z("canvas", { ref: g, style: Y });
}
export {
  re as default
};
